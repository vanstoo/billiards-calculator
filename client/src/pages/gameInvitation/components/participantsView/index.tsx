...
const ParticipantsView: React.FC<ParticipantsViewProps> = ({
  ...
  useEffect(() => {
    ...
    let newList = participants.map(({ startTime, endTime, ...val }) => {
      ...
      if (newStartTime) {
        newStartTime = formatDate(
          dayjs(`${formatDate(dayjs())} ${startTime}`),
          dateFormatToMin
        );
      }
      let newEndTime = endTime;
      if (newEndTime) {
        newEndTime = formatDate(
          dayjs(`${formatDate(dayjs())} ${endTime}`),
          dateFormatToMin
        );
        // 若起始时间都存在 判断结束时间是否为第二天
        if (newStartTime && dayjs(newEndTime).isBefore(dayjs(newStartTime))) {
          newEndTime = formatDate(
            dayjs(newEndTime).add(1, "d"),
            dateFormatToMin
          );
        }
      }
      ...
      let timeDuration =
        newEndTime && newStartTime
          ? dayjs(newEndTime).diff(dayjs(newStartTime), "m")
          : 0;
      return {
            ...
            {particapantList.map((item, index) => (
              <View key={index}>
                <View className="participant-info">
                  ...
                  <SectionItem
                    label="起始时间："
                    content={formatDate(item?.startTime, "HH:mm")}
                  />
                  <SectionItem
                    label="结束时间："
                    content={formatDate(item?.endTime, "HH:mm")}
                  />
                  <SectionItem
                    label="时长/占比："
                    content={returnDurationAndPercent(item?.duration)}
                  />
                  {totalFee ? (
                    <SectionItem
                      label="费用"
                      content={returnsingleFee(item?.duration)}
                    />
                  ) : null}
                </View>
                {particapantList.length - 1 > index && (
                  <View className="divider" />
                )}
              </View>
            ))}
